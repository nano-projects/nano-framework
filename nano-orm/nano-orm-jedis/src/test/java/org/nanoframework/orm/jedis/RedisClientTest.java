...
public class RedisClientTest {
    ...
    public void test0() {
        ...
        LOGGER.debug("get test0: " + redisClient.get(key));
        redisClient.del(key);
        boolean exists;
        LOGGER.debug("exists test0: " + (exists = redisClient.exists(key)));
    ...
    public void test1() throws InterruptedException {
        ...
        LOGGER.debug("exists test1: " + redisClient.exists(key));
        redisClient.expire(key, 1);
        LOGGER.debug("expire test1: 1s");
        Thread.sleep(2000L);
        boolean exists;
        LOGGER.debug("exists test1: " + (exists = redisClient.exists(key)));
    ...
    public void test2() throws InterruptedException {
        ...
        LOGGER.debug("exists test2: " + redisClient.exists(key));
        long now = System.currentTimeMillis() + 1000;
        redisClient.expireat(key, now);
        LOGGER.debug("expireAt test2: " + (now / 1000));
        long time = redisClient.ttl(key);
        LOGGER.debug("TTL Time has: " + time);
        if (time > 0)
            Thread.sleep((time + 1) * 1000);

        boolean exists;
        LOGGER.debug("exists test2: " + (exists = redisClient.exists(key)));

        if (exists) {
            LOGGER.error(key + ": redis key 没有过期");
            redisClient.del(key);
            if (redisClient.exists(key))
    ...
    public void test3() {
        ...
        LOGGER.debug("exists test3: " + redisClient.exists(key));
        LOGGER.debug("keys: " + redisClient.keys("test*"));
        redisClient.del(key);
    ...
    public void test4() {
        ...
        LOGGER.debug("test4 value: " + redisClient.get(key));
    ...
    public void test5() throws InterruptedException {
        ...
        LOGGER.debug("test5 value: " + redisClient.get(key, new TypeReference<Map<String, String>>() {
        }));
        redisClient.del(key);
        if (redisClient.exists(key))
            throw new RedisClientException("redis key 没有删除");
        ...
        LOGGER.debug("test5 value: " + redisClient.get(key));
        redisClient.del(key);
        if (redisClient.exists(key))
            throw new RedisClientException("redis key 没有删除");
        ...
        redisClient.set(test5);
        LOGGER.debug("test5 value: " + redisClient.get(key + "-1", key + "-2", key + "-3"));
        redisClient.getset(key + "-2", "hello, test5-2 to update");
        LOGGER.debug("test5-2 value: " + redisClient.get(key + "-2"));
        ...
        try {
            ...
            if (changed > 0)
                LOGGER.debug("set by nx: OK");
            else
                LOGGER.debug("set by nx: EXISTS");

        } catch (Exception e) {
            LOGGER.error("set by nx: " + e.getMessage());
        }
        ...
        try {
            ...
            LOGGER.error("set by nx on Object: " + e.getMessage());
        }
        ...
        try {
            ...
            if (exists.size() > 0)
                LOGGER.debug("set by nx: FAIL, value: " + exists);
            else
                LOGGER.debug("set by nx: OK");
    ...
    public void test6() {
        ...
        LOGGER.debug("test6 value length: " + redisClient.strLen(key));
        redisClient.del(key);
        if (redisClient.exists(key))
            throw new RedisClientException("redis key 没有删除");
    ...
    public void test7() {
        ...
        LOGGER.debug("test7 value: " + redisClient.hget(key, "field-1"));
        LOGGER.debug("test7 value exists: " + redisClient.hexists(key, "field-1"));
        redisClient.hset(key, "field-2", "value-2");
        LOGGER.debug("test7 values: " + redisClient.hmget(key, "field-1", "field-2"));
        redisClient.del(key);
        if (redisClient.exists(key))
            throw new RedisClientException("redis key 没有删除");
        ...
        Map<String, Map<String, String>> map = redisClient.hmget(key, new String[] { "field-1", "field-3" },
                new TypeReference<Map<String, String>>() {
                });
        LOGGER.debug("test7 hash values: " + map);
        LOGGER.debug("test7 hgetAll values: " + redisClient.hgetAll(key));

        map = redisClient.hgetAll(key, new TypeReference<Map<String, String>>() {
        });
        LOGGER.debug("test7 hgetAll values: " + map);

        LOGGER.debug("test7 hkeys: " + redisClient.hkeys(key));
        LOGGER.debug("test7 hlen: " + redisClient.hlen(key));

        try {
            redisClient.hsetByNX(key, "field-1", "value");
        } catch (Exception e) {
            LOGGER.error("hset nx: " + e.getMessage());
        }

        LOGGER.debug("test7 hvals: " + redisClient.hvals(key));
        redisClient.del(key);
