...
public class AtomicLongTest {
	...
	private Logger LOG = LoggerFactory.getLogger(AtomicLongTest.class);
	...
	public void loopSingleTest() {
		...
		AtomicLong longer = new AtomicLong(redisClient);
		for(int i = 0; i < 100_000; i ++) {
			long value = longer.getAndIncrement();
			if(i % 100 == 0)
		...
		for(int i = 0; i < parallel; i ++) {
			pool.execute(() -> {
				...
				for(int idx = 0; idx < 100; idx ++) {
					long value = longer.incrementAndGet();
					if(idx % 10 == 0)
						LOG.debug("now value: {}", value);
				}
